...
public class ProduitService implements IDao<Produit> {
    ...
    public List<Produit> findByCategorie(Categorie c) {
        ...
        List<Produit> desProduits;
        List<Produit> produits = null;
        desProduits = cs.findById(c.getId()).getProduits();
        for (Produit e : desProduits) {
            produits.add(e);
        }
        return produits;
    }

    public List<Produit> listProduitEntreDate(Date d1, Date d2) {
        List<Produit> produits = null;
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            Query qeury = session.getNamedQuery("listProduitEntreDate");
    ...
    public List<Produit> afficherProduitsPrixSuperieur() {
        
        double prixSeuil = 100;
        List<Produit> produits = null;
        ...
        try {
            ...
            Query query = session.getNamedQuery("listProduitEntreDate");
            produits = query.list();
            query.setParameter("prix", prixSeuil);
            produits = query.list();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
